"""Memory recall tool extractor with LLM-powered structured JSON output."""
...
class MemoryRecallExtractor:
    ...
    async def extract(
        ...
        if settings.per_tool_extraction_enabled:
            system_prompt = (
                "You are an expert at analyzing memory recall queries. "
                "Return ONLY valid JSON with keys: query, summary."
            )
            user_prompt = f"Memory recall query: {query}"

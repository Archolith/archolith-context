...
Injects a synthetic `__context_engine_recall` tool into requests when a
...
2. After upstream responds, check if the model called __context_engine_recall
...
RECALL_TOOL_NAME = "__context_engine_recall"


def inject_recall_tool(body: dict[str, Any]) -> dict[str, Any]:
    """Inject the __context_engine_recall tool into the request body.
...
def strip_recall_tool(body: dict[str, Any]) -> dict[str, Any]:
    """Remove the __context_engine_recall tool from the request body.
...
def strip_recall_from_response(response_data: dict[str, Any]) -> dict[str, Any]:
    """Remove __context_engine_recall tool calls from the response.
...
async def handle_recall_tool_call(
    ...
    """Handle a __context_engine_recall tool call by querying the session graph.
